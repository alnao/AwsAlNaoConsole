...
def job_detail(profile_name,job_name):
    boto3.setup_default_session(profile_name=profile_name)
    client = boto3.client('glue')
...
def job_execution_list(profile_name,job_name):
    boto3.setup_default_session(profile_name=profile_name)
    client = boto3.client('glue')
...
def job_execution_detail(profile_name,job_name,run_id):
    boto3.setup_default_session(profile_name=profile_name)
    client = boto3.client('glue')
...
def main():
    ...
    print("-----------")
    if len(l) > 0:
        print ( job_detail("default" , l[0]["Name"]) )
        e=job_execution_list("default", l[0]["Name"])
        if len(e)>0:
            print("-----------")
            print(e[0])
            print("-----------")
            r=job_execution_detail("default" , l[0]["Name"] , e[0]["Id"] )
            print( r )
    print("----------- END")
